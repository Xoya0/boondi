...
function UsersTab() {
  ...
  return (
    <>
      {page.items.map(user => (
        <div key={user.id} className="flex items-center gap-3 px-4 py-3 border-b border-stone-100">
          <Avatar src={user.profilePictureUrl} alt={user.displayName ?? user.username} size="sm" />
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-stone-900 text-sm truncate">
              {user.displayName ?? user.username}
              {user.role === 'ADMIN' && (
                <span className="ml-2 text-[10px] font-bold text-brand-600 bg-brand-50 px-1.5 py-0.5 rounded">
                  ADMIN
                </span>
              )}
            </p>
        ...
        <div key={report.id} className="px-4 py-3 border-b border-stone-100">
          ...
          {report.reportedUser && (
            <button
              ...
              className="mt-2 text-sm text-brand-600 hover:underline cursor-pointer block"
            >
